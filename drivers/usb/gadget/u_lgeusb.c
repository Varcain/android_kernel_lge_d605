...
struct lgeusb_dev {
	...
	int (*get_cable_adc)(struct chg_cable_info *info);
};
		...
		struct device_attribute *attr, char *buf)
{
	...
	if(info.cable_type >= NO_INIT_CABLE && info.cable_type <=CABLE_NONE)
        return sprintf(buf, "%s:TA(%d)USB(%d)ADC(%d)TH(%d)\n", (char*)strCable[info.cable_type], info.ta_ma, info.usb_ma, info.adc, info.threshould);
    else
        return -1;
}
...
static int __init lgeusb_probe(struct platform_device *pdev)
{
	...
	if (pdata) {
		...
		if (pdata->get_cable_adc)
			usbdev->get_cable_adc = pdata->get_cable_adc;
	}
