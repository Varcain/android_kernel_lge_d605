...
typedef struct dhd_info {
	...
	struct pm_qos_request dhd_dma_qos;
...
dhd_stop(struct net_device *net)
{
	...
	pm_qos_remove_request(&dhd->dhd_dma_qos);
	DHD_ERROR(("%s : dhd_open: dhd_dma_qos remove \n", __FUNCTION__));
...
dhd_open(struct net_device *net)
{
	...
	pm_qos_add_request(&dhd->dhd_dma_qos, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);
	pm_qos_update_request(&dhd->dhd_dma_qos,  2);  
	DHD_ERROR(("%s : dhd_open: dhd_dma_qos PM_QOS_CPU_DMA_LATENCY = %d \n", __FUNCTION__,  pm_qos_request(PM_QOS_CPU_DMA_LATENCY)));
	DHD_ERROR(("%s : dhd_open: dhd_dma_qos PM_QOS_NETWORK_LATENCY = %d \n", __FUNCTION__,  pm_qos_request(PM_QOS_NETWORK_LATENCY)));
