 ...
 *
 * $Id: wl_cfg80211.c 401768 2013-05-13 17:00:50Z $
 */
/* */
#include <typedefs.h>
#include <linuxver.h>
...
/* wlcfg_drv_priv should be static. Do not change */
	...
	const wl_event_msg_t *e, void *data)
{
	...
	if (!ndev || (!wl_get_drv_status(wl, SCANNING, ndev) && !wl->sched_scan_running)) {
		WL_ERR(("escan is not ready ndev %p drv_status 0x%x e_type %d e_states %d\n",
			ndev, wl_get_drv_status(wl, SCANNING, ndev),
			ntoh32(e->event_type), ntoh32(e->status)));
		goto exit;
	}
...
int wl_cfg80211_scan_stop(struct net_device *net)
{
	struct wl_priv *wl = wlcfg_drv_priv;
	unsigned long flags;
	int clear_flag = 0;
...
bool wl_cfg80211_get_vsdb_mode()
{
