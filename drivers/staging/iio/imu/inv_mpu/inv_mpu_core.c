	...
	struct device_attribute *attr, char *buf)
{
	...
	case ATTR_REG_WRITE:
        ...
        case ATTR_GYRO_CALI:
            result = inv_gyro_cal_self_test(st);
			printk("[MPU] Gyro Sensor Calibration end. return value : %d\n", result);
	    return sprintf(buf, "%d\n", result);
        case ATTR_ACCEL_CALI:
            result = inv_accel_cal_self_test(st);
			printk("[MPU] Accelerometer Sensor Calibration end. return value : %d\n", result);
            return sprintf(buf, "%d\n", result);
		case ATTR_GYRO_SW_SELF_TEST:
		{
	        int gyro_bias[3] = {0}; //absolute gyro bias scaled by 1000 times. (gyro_bias x 1000)
	        int gyro_rms[3] = {0}; //absolute gyro rms scaled by 1000 times. (gyro_bias x 1000)
	        int gyro_lsb_bias[3] = {0};
	        int result = 0;

	        result = inv_sw_self_test(st, gyro_bias, gyro_rms, gyro_lsb_bias, 0);
...
/*
	        return sprintf(buf, "%d "
		        ...
		        "%d.%03d %d.%03d %d.%03d ",
	            result,
	            (int)abs(gyro_bias[0]/1000), (int)abs(gyro_bias[0])%1000,
	            (int)abs(gyro_bias[1]/1000), (int)abs(gyro_bias[1])%1000,
	            (int)abs(gyro_bias[2]/1000), (int)abs(gyro_bias[2])%1000,
	            gyro_rms[0]/1000, (int)abs(gyro_rms[0])%1000,
	            gyro_rms[1]/1000, (int)abs(gyro_rms[1])%1000,
	            gyro_rms[2]/1000, (int)abs(gyro_rms[2])%1000
	            );
*/
	        }
		case ATTR_ACCEL_SW_SELF_TEST:
		{
	        int accel_bias[3] = {0}; //absolute accel bias scaled by 1000 times. (accel_bias x 1000)
	        int accel_rms[3] = {0}; //absolute accel rms scaled by 1000 times. (accel_bias x 1000)
	        int accel_lsb_bias[3] = {0};
	        int result = 0;

	        result = inv_sw_self_test(st, accel_bias, accel_rms, accel_lsb_bias, 1);
			return sprintf(buf, "%d", result);
	/*
				return sprintf(buf, "%d "
	            "%d.%03d %d.%03d %d.%03d "
	            "%d.%03d %d.%03d %d.%03d ",
	            result,
	            (int)abs(accel_bias[0]/1000), (int)abs(accel_bias[0])%1000,
	            (int)abs(accel_bias[1]/1000), (int)abs(accel_bias[1])%1000,
	            (int)abs(accel_bias[2]/1000), (int)abs(accel_bias[2])%1000,
	            accel_rms[0]/1000, (int)abs(accel_rms[0])%1000,
	            accel_rms[1]/1000, (int)abs(accel_rms[1])%1000,
	            accel_rms[2]/1000, (int)abs(accel_rms[2])%1000
	            );
	*/
	    }

		case ATTR_HW_SELF_TEST:
			result = inv_hw_self_test(st);
			printk("Accel Gyro self test return value : %d\n", result);
			return sprintf(buf, "%d\n", result);
				 ...
				 struct iio_buffer *ring, bool en)
{
	...
	if(st->accel_load_bias!=1) {
		inv_accel_load_cal(st);
		st->accel_load_bias = 1;
	}
...
#ifdef FEATURE_IKR_SELF_TEST
static IIO_DEVICE_ATTR(gyro_cali, S_IRUGO, inv_attr_show, NULL,
	ATTR_GYRO_CALI);
static IIO_DEVICE_ATTR(accel_cali, S_IRUGO, inv_attr_show, NULL,
	ATTR_ACCEL_CALI);
static IIO_DEVICE_ATTR(sw_gyro_self_test, S_IRUGO, inv_attr_show, NULL,
	ATTR_GYRO_SW_SELF_TEST);
static IIO_DEVICE_ATTR(sw_accel_self_test, S_IRUGO, inv_attr_show, NULL,
	ATTR_ACCEL_SW_SELF_TEST);
static IIO_DEVICE_ATTR(hw_self_test, S_IRUGO, inv_attr_show, NULL,
	ATTR_HW_SELF_TEST);
#endif //FEATURE_IKR_SELF_TEST
...
#ifdef FEATURE_IKR_SELF_TEST
	&iio_dev_attr_gyro_cali.dev_attr.attr,
	&iio_dev_attr_accel_cali.dev_attr.attr,
	&iio_dev_attr_sw_gyro_self_test.dev_attr.attr,
	&iio_dev_attr_sw_accel_self_test.dev_attr.attr,
	&iio_dev_attr_hw_self_test.dev_attr.attr,
#endif // FEATURE_IKR_SELF_TEST
};
