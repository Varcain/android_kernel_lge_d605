...
#define DEF_SELFTEST_GYRO_FULL_SCALE         (0 << 3)  // 250 dps
#define DEF_SELFTEST_ACCL_FULL_SCALE         (2 << 3)  // 8G
...
#ifdef FEATURE_IKR_SELF_TEST
#define DEF_RMS_SCALE_FOR_RMS (10000)
#define DEF_SQRT_SCALE_FOR_RMS (100)
#define DEF_BIAS_THRESH_SELF     (40)	/* dps */
#define DEF_BIAS_LSB_THRESH_SELF (DEF_BIAS_THRESH_SELF*DEF_SELFTEST_GYRO_SENS)
#define DEF_RMS_THRESH           (5 * DEF_SELFTEST_GYRO_SENS)

#define DEF_ACCL_XY_BIAS_THRESH_SELF     (269)	/* mg */
#define DEF_ACCL_Z_BIAS_THRESH_SELF     (319)	/* mg */

#define DEF_ACCL_XY_BIAS_LSB_THRESH_SELF (DEF_ACCL_XY_BIAS_THRESH_SELF*DEF_SELFTEST_ACCL_SENS)
...
static struct test_setup_t test_setup = {
    ...
    .accel_sens     = DEF_SELFTEST_ACCL_SENS,
	...
	.rms_thresh = DEF_RMS_THRESH*DEF_RMS_THRESH,
	.accl_z_bias_thresh  = DEF_ACCL_Z_BIAS_LSB_THRESH_SELF,
	.accl_xy_bias_thresh  = DEF_ACCL_XY_BIAS_LSB_THRESH_SELF,
	.accl_rms_thresh = DEF_RMS_THRESH*DEF_RMS_THRESH,
#endif //FEATURE_IKR_SELF_TEST
};
...
static int inv_check_accl_self_test(struct inv_mpu_iio_s *st,
	...
	if (st->chip_info.software_revision < DEF_OLDEST_SUPP_SW_REV &&
		...
		printk("software_revision : %d, product_revision : %d\n",
				st->chip_info.software_revision, st->chip_info.product_revision);
		return 0;
	}
...
/**
 *  inv_gyro_cal() - do the actual test of self testing
 */
int inv_gyro_cal(struct inv_mpu_iio_s *st, int *gyro_result)
{
	struct inv_reg_map_s *reg;
	int result, i, j, packet_size;
	u8 data[BYTES_PER_SENSOR];
	int fifo_count, packet_count;
	int ret = 0;
	int gyro_data[3] = {0};
	...
	printk("packet_count : %d, gyro_data: %d %d %d\n", packet_count, gyro_data[0], gyro_data[1], gyro_data[2]);
	gyro_data[0] = gyro_data[0] * DEF_ST_PRECISION / packet_count;
	gyro_data[1] = gyro_data[1] * DEF_ST_PRECISION / packet_count;
	gyro_data[2] = gyro_data[2] * DEF_ST_PRECISION / packet_count;
	printk("gyro_data: %d %d %d\n", gyro_data[0], gyro_data[1], gyro_data[2]);

	for (i = 0; i < 3; i++) {
		if (abs(gyro_data[j]) > test_setup.bias_thresh) {
			result |= (1 << i);
		}
	}

	if(ret == 0){
		gyro_result[0] = gyro_data[0];
		gyro_result[1] = gyro_data[1];
		gyro_result[2] = gyro_data[2];
	}

	printk("gyro_result: %d %d %d\n", gyro_result[0], gyro_result[1], gyro_result[2]);
	printk("test_setup.bias_thresh:%d\n", test_setup.bias_thresh);
	
	return 0;
}


int inv_accel_load_cal(struct inv_mpu_iio_s *st)
{
	int fd = 0;
	mm_segment_t old_fs = get_fs();
	set_fs(KERNEL_DS);

	fd = sys_open(DEF_ACCL_CAL_FILE, O_RDONLY, 0);
	if(fd >= 0) {
		sys_read(fd,(char __user *)st->driver_accel_bias,sizeof(st->driver_accel_bias));
		sys_close(fd);
	}
	set_fs(old_fs);
	return 0;
...
int inv_accel_store_cal(struct inv_mpu_iio_s *st)
{
	mm_segment_t old_fs = get_fs();
	loff_t pos = 0;
	int fd;
	...
	set_fs(KERNEL_DS);
	fd = sys_open(DEF_ACCL_CAL_FILE, O_WRONLY|O_CREAT, 0644);
	printk("accel data store fd : %d\n", fd);
	if(fd >= 0) {
		sys_write(fd,(const char __user *)st->driver_accel_bias,sizeof(st->driver_accel_bias));
		file = fget(fd);
		if(file) {
			vfs_write(file,(const char __user *)st->driver_accel_bias,sizeof(st->driver_accel_bias),&pos);
			fput(file);
		}
		sys_close(fd);
	}
 ...
 *  inv_do_test() - do the actual test of self testing
 */
int inv_accel_cal(struct inv_mpu_iio_s *st, int *accl_result)
{
	struct inv_reg_map_s *reg;
	int result, i, j, packet_size;
	u8 data[BYTES_PER_SENSOR];
	...
	reg = &st->reg;
	packet_size = BYTES_PER_SENSOR;
	memset(accl_result, 0, 3*sizeof(int));
	...
	if (result)
		return result;
	/* enable sensor output to FIFO */
	result = inv_i2c_single_write(st, reg->fifo_en, BIT_ACCEL_OUT);
	...
	printk("packet_count : %d, accel_data: %d %d %d\n", packet_count, accel_data[0], accel_data[1], accel_data[2]);

	accel_data[0] = accel_data[0] / packet_count;
	accel_data[1] = accel_data[1] / packet_count;
	accel_data[2] = accel_data[2] / packet_count
					- 16384*st->plat_data.orientation[8];

	printk("accel_data: %d %d %d\n", accel_data[0], accel_data[1], accel_data[2]);

    if ((abs(accel_data[0])*DEF_ST_PRECISION) > test_setup.accl_xy_bias_thresh) {
		ret |= 1;
	}
	if ((abs(accel_data[1])*DEF_ST_PRECISION) > test_setup.accl_xy_bias_thresh) {
		ret |= 2;
	}
	if ((abs(accel_data[2])*DEF_ST_PRECISION) > test_setup.accl_z_bias_thresh) {
		ret |= 4;
	}

	if(ret == 0){
		accl_result[0] = accel_data[0];
		accl_result[1] = accel_data[1];
		accl_result[2] = accel_data[2];
	}

	printk("accl_result: %d %d %d\n", accl_result[0], accl_result[1], accl_result[2]);
	printk("test_setup.accl_bias_thresh - xy_axis:%d,  z_axis:%d\n", test_setup.accl_xy_bias_thresh, test_setup.accl_z_bias_thresh);
...
{
	int result;
	int gyro_bias_st[THREE_AXIS];
	int gyro_bias_regular[THREE_AXIS] = {0,};
	int accl_bias_st[THREE_AXIS];
	int accl_bias_regular[THREE_AXIS] = {0,};
	int test_times;
	...
	printk("accel_result : %d, gyro_result : %d return_value : %d\n", accel_result, gyro_result,
...
/**
 *  inv_accel_cal_self_test() - main function to do hardware self test
 */
int inv_accel_cal_self_test(struct inv_mpu_iio_s *st)
{
	int count=3;
	int result;
	int test_times;
    ...
    printk(KERN_INFO "inv_accel_cal_self_test\n");
	while (test_times > 0 && count > 0) {
		result = inv_accel_cal(st, st->driver_accel_bias);
		if(result > 0){
			count--;
			printk("Accelerometer Calibration Fail. Retry cnt : %d", 3-count);
			continue;
		}
		if (result == -EAGAIN)
			test_times--;
	...
	inv_accel_store_cal(st);
	printk(KERN_INFO "inv_accel_cal_self_test end. result:%d\n", result);
...
/**
 *  inv_gyro_cal_self_test() - main function to do hardware self test
 */
int inv_gyro_cal_self_test(struct inv_mpu_iio_s *st)
{
	int count=3;
	int result;
	int test_times;
    ...
    printk(KERN_INFO "Gyro Calibration start\n");
	while (test_times > 0) {
		result = inv_gyro_cal(st,st->gyro_bias);
		if(result > 0){
			count--;
			printk("Gyro Calibration Fail. Retry cnt : %d", 3-count);
			continue;
		}
		if (result == -EAGAIN)
			test_times--;
	...
	printk(KERN_INFO "Gyro Calibration end. result:%d\n", result);
...
#ifdef FEATURE_IKR_SELF_TEST
int inv_sw_self_test(struct inv_mpu_iio_s *st,int* gyro_bias_regular,int* gyro_rms_regular,int* gyro_lsb_bias_regular,int sensor)
{
	int result = 0;
	int test_times;
	char gyro_result;
    printk(KERN_INFO "is_asleep =%d, lpa_mode=%d, gyro_enable=%d\n",st->chip_config.is_asleep,st->chip_config.lpa_mode,st->chip_config.gyro_enable);
	...
	while (test_times > 0) {
        if(sensor == 1)
        {
		    result = inv_accel_test(st, gyro_bias_regular,
			    gyro_rms_regular, gyro_lsb_bias_regular);
		    if (result == -EAGAIN)
			    test_times--;
		    else
			    test_times = 0;
        ...
        {
		result = inv_gyro_test(st, gyro_bias_regular,
			gyro_rms_regular, gyro_lsb_bias_regular);
		if (result == -EAGAIN)
			test_times--;
		else
...
/**
 *  inv_gyro_test() - do the actual test of S/W self testing
 */
int inv_gyro_test(struct inv_mpu_iio_s *st,
		int *gyro_bias, int *gyro_rms, int *gyro_lsb_bias)
{
	struct inv_reg_map_s *reg;
	int result = 0, i = 0,packet_size = 0,j = 0;
	...
	// Low pass filter set to 1 , 188hz, 1.9ms
	result = inv_i2c_single_write(st, reg->lpf, test_setup.lpf);
    ...
    printk(KERN_INFO "packet count = %d\n", packet_count);

	x = kmalloc(packet_count*sizeof(int), GFP_KERNEL);
	y = kmalloc(packet_count*sizeof(int), GFP_KERNEL);
	z = kmalloc(packet_count*sizeof(int), GFP_KERNEL);

    memset(x, 0, packet_count*sizeof(int));
    memset(y, 0, packet_count*sizeof(int));
    memset(z, 0, packet_count*sizeof(int));
	...
	for (i = 0; i < packet_count; i++) {
        ...
        x[i] = (short)be16_to_cpup(
					(__be16 *)(&data[ind]));
        y[i] = (short)be16_to_cpup(
					(__be16 *)(&data[ind+2]));
        z[i] = (short)be16_to_cpup(
					(__be16 *)(&data[ind+4]));
        printk("x = %d, y = %d , z = %d\n", x[i],y[i], z[i]);
	}
    ...
    for (i = 0, avg[X] = 0, avg[Y] = 0, avg[Z] = 0;
          i < packet_count; i++) {
         avg[X] += x[i];
         avg[Y] += y[i];
         avg[Z] += z[i];
    }
    avg[X] = avg[X]/packet_count;
    avg[Y] = avg[Y]/packet_count;
    avg[Z] = avg[Z]/packet_count;

	for (j = 0; j < 3; j++) {
		if (abs(avg[j]) > test_setup.bias_thresh) {
			printk(KERN_INFO "%d-Gyro bias (%ld) exceeded threshold "
			       "(threshold = %d LSB)\n", j,
    ...
    gyro_bias[X] = avg[X] * DEF_ST_PRECISION / test_setup.gyro_sens;
    gyro_bias[Y] = avg[Y] * DEF_ST_PRECISION / test_setup.gyro_sens;
    gyro_bias[Z] = avg[Z] * DEF_ST_PRECISION / test_setup.gyro_sens;

    gyro_lsb_bias[X] = avg[X];
    gyro_lsb_bias[Y] = avg[Y];
    gyro_lsb_bias[Z] = avg[Z];
    //gyro_bias[X] = gyro_bias[X] / packet_count;
    //gyro_bias[Y] = gyro_bias[Y] / packet_count;
    //gyro_bias[Z] = gyro_bias[Z] / packet_count;
    printk("bias : %+8ld %+8ld %+8ld (LSB)\n", (long int)gyro_lsb_bias[X],
               (long int)gyro_lsb_bias[Y], (long int)gyro_lsb_bias[Z]);

	printk("abs bias : %+8d.%02d   %+8d.%02d  %+8d.%02d (dps)\n",
		     (int)abs(gyro_bias[X]) / DEF_ST_PRECISION,
		     (int)abs(gyro_bias[X]) % DEF_ST_PRECISION,
		     (int)abs(gyro_bias[Y]) / DEF_ST_PRECISION,
		     (int)abs(gyro_bias[Y]) % DEF_ST_PRECISION,
		     (int)abs(gyro_bias[Z]) / DEF_ST_PRECISION,
		     (int)abs(gyro_bias[Z]) % DEF_ST_PRECISION);

    // Calcuate Gyro RMS
    for (i = 0,
              rms[X] = 0, rms[Y] = 0, rms[Z] = 0;
           i < packet_count; i++) {
   ...
   printk(KERN_INFO "rms : %ld %ld %ld (rms)\n",
                    rms[0],
                    rms[1],
                    rms[2]);
    for (j = 0; j < 3; j++) {
		if (rms[j]/packet_count > test_setup.rms_thresh) {
            printk(KERN_INFO "%d -Gyro rms (%ld) exceeded threshold "
            "(threshold = %d LSB)\n", j,
            (long)rms[j]/packet_count, (int)test_setup.rms_thresh);
            result |= 1 << (7 + j);
            //goto gyro_test_fail;
        }
    }

    {
        ...
        for(i = 0; i < 3; i++) {
			if(rms[i] > 10000) {
                tmp = ((u32)(rms[i]/packet_count));

                printk(KERN_INFO " %d = temp : %u\n",i,
                            tmp);
				tmp = tmp*DEF_RMS_SCALE_FOR_RMS;
                printk(KERN_INFO " %d = temp : %u\n",i,
                            tmp);
			} else {
			...
			dps_rms[i] = inv_sqrt(tmp)/test_setup.gyro_sens;

			gyro_rms[i] = dps_rms[i]*DEF_ST_PRECISION/DEF_SQRT_SCALE_FOR_RMS;
        }

        printk(KERN_INFO "dps_rms : %d %d %d (dps_rms)\n",
                    dps_rms[0],
                    dps_rms[1],
                    dps_rms[2]);

        printk(KERN_INFO "RMS : %+8d.%02d	 %+8d.%02d  %+8d.%02d (dps)\n",
			(int)abs(gyro_rms[X]) / DEF_ST_PRECISION,
			(int)abs(gyro_rms[X]) % DEF_ST_PRECISION,
			(int)abs(gyro_rms[Y]) / DEF_ST_PRECISION,
			(int)abs(gyro_rms[Y]) % DEF_ST_PRECISION,
			(int)abs(gyro_rms[Z]) / DEF_ST_PRECISION,
			(int)abs(gyro_rms[Z]) % DEF_ST_PRECISION);
	}
...
/**
 *  inv_accel_test() - do the actual test of S/W self testing
 */
int inv_accel_test(struct inv_mpu_iio_s *st,
		int *accl_bias, int *accl_rms, int *accl_lsb_bias)
{
	struct inv_reg_map_s *reg;
	int result = 0, i = 0,packet_size = 0,j = 0;
	unsigned char data[BYTES_PER_SENSOR * 2];
    ...
    gravity = (u32)(DEF_ST_SCALE *
                            DEF_ST_PRECISION / DEF_ST_ACCL_FULL_SCALE);
    printk(KERN_INFO "jinugi !! gravity = %d\n",gravity);
    ...
    printk("average : %+8ld %+8ld %+8ld (LSB)\n", avg[X],
                  avg[Y], avg[Z]);

    // z bias => 1G - avg[Z]

    avg[Z] = _g_z_sign*avg[Z] - gravity;
	...
	for (j = 0; j < 3; j++) {
        ...
        if(j!=2)
        {
		    if ((abs(avg[j])*DEF_ST_PRECISION) > test_setup.accl_xy_bias_thresh) {
			    printk(KERN_INFO "%d-accel XY bias (%ld) exceeded threshold "
			       "(threshold = %+8d.%02d LSB)\n", j,
			       avg[j], test_setup.accl_xy_bias_thresh/DEF_ST_PRECISION,test_setup.accl_xy_bias_thresh%DEF_ST_PRECISION);
			    result |= 1 << (3 + j);
            //goto gyro_test_fail;
		   }
        }
        ...
        {
		    if ((abs(avg[j])*DEF_ST_PRECISION) > test_setup.accl_z_bias_thresh) {
			    printk(KERN_INFO "%d-accel Z bias (%ld) exceeded threshold "
			       "(threshold = %+8d.%02d LSB)\n", j,
			       avg[j], test_setup.accl_z_bias_thresh/DEF_ST_PRECISION,test_setup.accl_z_bias_thresh%DEF_ST_PRECISION);
			    result |= 1 << (3 + j);
    ...
    accl_bias[X] = avg[X] * DEF_ST_PRECISION / test_setup.accel_sens;
    accl_bias[Y] = avg[Y] * DEF_ST_PRECISION / test_setup.accel_sens;
    accl_bias[Z] = avg[Z] * DEF_ST_PRECISION / test_setup.accel_sens;

    accl_lsb_bias[X] = avg[X];
    accl_lsb_bias[Y] = avg[Y];
    accl_lsb_bias[Z] = avg[Z];
    //gyro_bias[X] = gyro_bias[X] / packet_count;
    //gyro_bias[Y] = gyro_bias[Y] / packet_count;
    //gyro_bias[Z] = gyro_bias[Z] / packet_count;
    printk("bias : %+8ld %+8ld %+8ld (LSB)\n", (long int)accl_lsb_bias[X],
               (long int)accl_lsb_bias[Y], (long int)accl_lsb_bias[Z]);

	printk("abs bias : %+8d.%02d   %+8d.%02d  %+8d.%02d (G)\n",
		     (int)abs(accl_bias[X]) / DEF_ST_PRECISION,
		     (int)abs(accl_bias[X]) % DEF_ST_PRECISION,
		     (int)abs(accl_bias[Y]) / DEF_ST_PRECISION,
		     (int)abs(accl_bias[Y]) % DEF_ST_PRECISION,
		     (int)abs(accl_bias[Z]) / DEF_ST_PRECISION,
		     (int)abs(accl_bias[Z]) % DEF_ST_PRECISION);
